...
class TestServerImports:
    ...
    def test_can_import_server(self):
        ...
        with patch('codex_logbook.utils.cache_warmer.warm_recent_projects', new=AsyncMock()):
            import codex_logbook.server
            assert hasattr(codex_logbook.server, 'app')
            assert hasattr(codex_logbook.server, 'memory_cache')
            assert hasattr(codex_logbook.server, 'cache_service')
...
class TestServerEndpointStructure:
    ...
    def test_server_has_required_endpoints(self):
        ...
        with patch('codex_logbook.utils.cache_warmer.warm_recent_projects', new=AsyncMock()):
            from codex_logbook.server import app
    ...
    def test_server_global_variables(self):
        ...
        with patch('codex_logbook.utils.cache_warmer.warm_recent_projects', new=AsyncMock()):
            import codex_logbook.server as server
...
class TestCacheWarmerFunction:
    ...
    async def test_warm_recent_projects_handles_empty_dir(self):
        ...
        with patch('codex_logbook.utils.cache_warmer.get_all_projects_with_metadata', return_value=[]):
    ...
    async def test_warm_recent_projects_skips_current(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            project1 = export_dir / "project1"
            project1.mkdir()
            (project1 / "log.jsonl").write_text('{"type": "user"}\n')

            project2 = export_dir / "project2"
            project2.mkdir()
            ...
            with patch('codex_logbook.utils.cache_warmer.get_all_projects_with_metadata', return_value=projects):
                with patch('codex_logbook.utils.cache_warmer.CodexLogProcessor') as mock_processor:
...
class TestServerConfiguration:
    ...
    def test_cache_configuration_from_env(self):
        ...
        with patch.dict(os.environ, {
            ...
            import codex_logbook.server
            importlib.reload(codex_logbook.server)
            
            assert codex_logbook.server.max_projects == 10
            assert codex_logbook.server.max_mb_per_project == 100
            assert codex_logbook.server.cache_warm_on_startup == 5
...
class TestProjectAPIMethods:
    ...
    def test_project_path_conversion(self):
        ...
        dir_name = "-Users-developer-dev-demoapp"
        expected_path = "/Users/developer/dev/demoapp"
        ...
        mock_projects = [
            {
                ...
                'log_path': '/home/.codex-logbook/codex/projects/-Users-test-project1',
                'file_count': 3,
                'total_size_mb': 1.5,
            ...
            },
            {
                ...
                'log_path': '/home/.codex-logbook/codex/projects/-Users-test-project2',
                'file_count': 1,
                'total_size_mb': 0.5,
...
class TestRefreshAllProjects:
    ...
    async def test_refresh_all_projects_no_changes(self):
        ...
        from codex_logbook.server import refresh_all_projects
        ...
        with patch('codex_logbook.utils.log_finder.get_all_projects_with_metadata', return_value=test_projects):
            with patch('codex_logbook.server.cache_service', mock_cache_service):
                with patch('codex_logbook.server.memory_cache', mock_memory_cache):
    ...
    async def test_refresh_all_projects_with_changes(self):
        ...
        with patch('codex_logbook.utils.log_finder.get_all_projects_with_metadata', return_value=test_projects):
            with patch('codex_logbook.server.cache_service', mock_cache_service):
                with patch('codex_logbook.server.memory_cache', mock_memory_cache):
                    with patch('codex_logbook.server.CodexLogProcessor') as mock_processor:
    ...
    async def test_refresh_all_projects_empty_projects(self):
        ...
        with patch('codex_logbook.utils.log_finder.get_all_projects_with_metadata', return_value=[]):
            result = await refresh_all_projects({})
    ...
    async def test_refresh_endpoint_calls_refresh_all_projects(self):
        ...
        from codex_logbook.server import refresh_data
        
        # Mock current_log_path to be None
        with patch('codex_logbook.server.current_log_path', None):
            with patch('codex_logbook.server.refresh_all_projects', new=AsyncMock()) as mock_refresh_all:
