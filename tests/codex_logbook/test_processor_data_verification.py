#!/usr/bin/env python3
"""
Test the CodexLogProcessor against known characteristics of the test data.
...
class TestActualDataVerification(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.test_data_dir = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'mock-data', '-Users-developer-dev-demo-project')
    ...
    def test_model_usage(self):
        ...
        # Should only have gpt-5
        self.assertEqual(len(model_dist), 1, "Should have only one model")
        self.assertIn('gpt-5', model_dist)
        self.assertEqual(model_dist['gpt-5'], 13,
