#!/usr/bin/env python3
"""
Statistics generation module for Codex Logbook.

This module generates comprehensive statistics from processed Codex messages,
including:
...
class StatisticsGenerator:
    """Generates comprehensive statistics from processed Codex messages.
    ...
    def generate_statistics(self, messages: list[dict], timezone_offset_minutes: int = 0) -> dict:
        ...
        log_dir_name = Path(self.log_directory).name  # Full directory name like -Users-example-user-dev-...

        if "/.codex-logbook/codex/projects/" in self.log_directory:
            project_name = log_dir_name.replace("-", "/").split("/")[-1]
    ...
    def _estimate_tokens(self, text: str) -> float:
        ...
        if not text:
            return 0.0
        # Codex's tokenization is roughly 4 characters per token
