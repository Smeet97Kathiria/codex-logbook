"""
Global statistics aggregator for Codex Logbook.
...
class GlobalStatsAggregator:
    """Aggregates statistics across all Codex projects."""
    ...
    async def process_uncached_projects(self, projects: list[dict], limit: int = 5) -> int:
        ...
        for project in uncached_projects[:limit]:
            try:
                log_path = project["log_path"]
                await asyncio.to_thread(self._process_project_for_cache, log_path)
    ...
    def _process_project_for_cache(self, log_path: str) -> None:
