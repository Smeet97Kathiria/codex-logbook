...
def init(port, host, no_browser, clear_cache):
    ...
    if clear_cache:
        ...
        cache_dir = Path.home() / ".codex-logbook" / "cache"
        if cache_dir.exists():
    ...
    if should_open_browser:
        ...
        click.echo(f"\n✨✨✨ Codex Logbook dashboard opened at {url}✨✨✨\n")
    else:
        click.echo(f"\n✨✨✨ Codex Logbook running at http://{host}:{port}✨✨✨\n")
...
def version():
    ...
    click.echo(f"codex-logbook v{__version__}")
...
Usage Examples:
  ...
  codex-logbook init --host 0.0.0.0
  
  # Start without opening browser
  codex-logbook init --no-browser
  
  # Clear cache and start fresh
  codex-logbook init --clear-cache
...
For more information, visit: https://codex-logbook.dev
"""
    )


def is_first_run():
    """Check if this is the first time running Codex Logbook."""
    config_path = Path.home() / ".codex-logbook" / "config.json"
    return not config_path.exists()


def handle_first_run_setup():
    """Handle first-run setup"""
    click.echo("\n🍋 Welcome to Codex Logbook!")
    click.echo("Your Codex analytics dashboard\n")
