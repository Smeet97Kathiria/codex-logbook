"""
Share functionality for codex_logbook dashboards
"""
...
# Load .env.codex-logbook.dev for share configuration
...
if env_file.exists():
    print(f"Loading .env.codex-logbook.dev from: {env_file}")
    load_dotenv(env_file)
    print(f"After loading, ENV = {os.getenv('ENV')}")
else:
    print(f".env.codex-logbook.dev not found at: {env_file}")
...
class ShareManager:
    def __init__(self):
        ...
        if env == "DEV":
            ...
            self.r2_endpoint = os.getenv("SHARE_STORAGE_PATH", "/Users/developer/dev/codex-analysis/fake-r2")
            self.is_production = False
            logger.info(f"ShareManager: Development mode, base_url={self.base_url}")
        else:
            self.base_url = config.get("share_base_url", "https://codex-logbook.dev")
            ...
            if os.getenv("R2_ACCESS_KEY_ID"):
                ...
                self.r2_endpoint = os.getenv("R2_ENDPOINT", "https://r2.codex-logbook.dev")
            else:
                # PyPI users use public API
                self.r2_endpoint = config.get("share_api_url", "https://codex-logbook.dev")
    ...
    async def _upload_to_r2(self, share_id: str, data: dict[str, Any]):
        ...
        r2_bucket = os.getenv("R2_BUCKET_NAME", "codex-logbook-shares")
