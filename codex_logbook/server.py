#!/usr/bin/env python3
"""
FastAPI application for Codex analytics dashboard local mode.
"""
...
from codex_logbook.api.messages import get_messages_summary, get_paginated_messages
from codex_logbook.config import Config
from codex_logbook.core.processor import CodexLogProcessor
from codex_logbook.utils.codex_exporter import export_codex_project
from codex_logbook.utils.cache_warmer import warm_recent_projects
...
# Create FastAPI app
app = FastAPI(
    title="Codex Logbook - Local Mode",
    description="Analyze your Codex logs directly from your local machine",
    version=__version__,
)
...
def project_table_stats_from_stats(stats: dict[str, Any]) -> dict[str, Any]:
    """Extract the compact stats payload used by the projects table."""
...
def load_or_process_project_stats(log_path: str) -> dict[str, Any] | None:
    """Load project stats from cache, or process and cache them if missing."""
    cache_result = memory_cache.get(log_path)
    if cache_result:
        _, stats = cache_result
        return stats

    cached_stats = cache_service.get_cached_stats(log_path)
    cached_messages = cache_service.get_cached_messages(log_path)
    if cached_stats:
        if cached_messages:
            memory_cache.put(log_path, cached_messages, cached_stats)
        return cached_stats
...
def resolve_project_log_path(dir_name: str) -> str | None:
    """Resolve an exported Codex project directory name to its log path."""
    codex_log_path = export_codex_project(dir_name)
    if codex_log_path:
        return codex_log_path

    from codex_logbook.utils.log_finder import get_all_projects_with_metadata

    for project in get_all_projects_with_metadata():
        if project["dir_name"] == dir_name:
    ...
    while True:
        try:
            ...
            if uncached_projects:
                ...
                from codex_logbook.core.global_aggregator import GlobalStatsAggregator
...
async def project_dashboard(project_name: str):
    ...
    # The project_name is the exported Codex project directory name.
...
async def set_project(data: dict[str, str]):
    ...
    # Find Codex logs for this project
    log_path = find_logs(project_path)
    if not log_path or not os.path.exists(log_path):
        raise HTTPException(
            status_code=404,
            detail=f"Codex logs not found for project: {project_path}. Make sure you have used Codex with this project.",
        )
...
async def set_project_by_dir(data: dict[str, str]):
    ...
    # Build the log path. Codex projects are exported on demand.
    codex_log_path = resolve_project_log_path(dir_name)
    log_path = Path(codex_log_path) if codex_log_path else Path()
...
async def get_dashboard_data(timezone_offset: int = 0):
    ...
    if memory_result:
        ...
        if timezone_offset != 0:
            from codex_logbook.core.stats import StatisticsGenerator
...
async def get_jsonl_files(project: str | None = None):
    ...
    if project:
        log_path = resolve_project_log_path(project)
...
# Get recent projects from local assistant logs
@app.get("/api/recent-projects")
async def get_recent_projects():
    """Get list of recent projects from Codex logs"""
    try:
        from codex_logbook.utils.log_finder import get_all_projects_with_metadata
...
async def get_projects(
    ...
    """
    Get all available Codex projects with metadata.
        ...
        if include_stats:
            ...
            for project in projects:
                try:
                    ...
                    if project["stats"]:
                        logger.debug(f"Added stats for project {project['dir_name']}: {project['stats']}")
                except Exception as stats_error:
                    logger.error(
                        "Failed to load stats for %s: %s: %s",
                        project["dir_name"],
                        type(stats_error).__name__,
                        stats_error,
                    )
                    project["stats"] = None
...
async def get_pricing():
    ...
    try:
        from codex_logbook.services.pricing_service import PricingService
...
async def create_share_link(data: dict[str, Any], request: Request):
    ...
    try:
        from codex_logbook.share import ShareManager
