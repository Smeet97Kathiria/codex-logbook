...
from ..utils.pricing import DEFAULT_CODEX_PRICING
...
class PricingService:
    ...
    def __init__(self):
        self.cache_dir = Path.home() / ".codex-logbook" / "cache"
    ...
    def _fetch_from_litellm(self) -> dict | None:
        ...
        try:
            ...
            return self._transform_litellm_to_codex(litellm_data)
    ...
    def _transform_litellm_to_codex(self, litellm_data: dict) -> dict:
        """Transform LiteLLM format to our Codex pricing format."""
        result = {}

        for model_name, model_data in litellm_data.items():
            # Only process OpenAI models used by Codex.
        ...
        # If no OpenAI models found, return our defaults
        if not result:
