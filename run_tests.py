#!/usr/bin/env python3
"""
Test runner for Codex Logbook project.
...
def run_tests(args):
    ...
    if args.file:
        ...
        if args.module == "processor":
            cmd.append("tests/codex_logbook/core/test_processor.py")
        elif args.module == "stats":
            cmd.append("tests/codex_logbook/core/test_stats.py")
        elif args.module == "memory_cache":
            cmd.append("tests/codex_logbook/utils/test_memory_cache.py")
        elif args.module == "verification":
            cmd.append("tests/codex_logbook/test_processor_data_verification.py")
        elif args.module == "performance":
            cmd.append("tests/codex_logbook/test_performance.py")
        elif args.module == "admin":
            cmd.append("tests/codex-logbook-site/test_admin.py")
        else:
        ...
        if not args.include_performance:
            cmd.extend(["tests/", "--ignore=tests/codex_logbook/test_performance.py"])
        else:
            cmd.append("tests/")
...
def run_linting(args):
    ...
    if ruff_check.returncode == 0:
        # Run ruff
        result = run_command(
            ["ruff", "check", "codex_logbook/", "tests/"],
            "Running ruff linter"
        )
        if result.returncode != 0:
            success = False
...
def run_type_checking(args):
    ...
    result = run_command(
        ["mypy", "codex_logbook/", "--ignore-missing-imports"],
...
def generate_report(args):
    ...
    if not args.include_performance:
        cmd.append("--ignore=tests/codex_logbook/test_performance.py")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Run tests for Codex Logbook project",
...
Examples:
  ...
  python run_tests.py --report           # Generate test report
    ...
    # Header
    print_header("Codex Logbook Test Runner")
